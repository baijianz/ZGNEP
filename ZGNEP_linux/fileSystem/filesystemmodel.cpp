...
QMimeData *FileSystemModel::mimeData(const QModelIndexList &indexes) const
{
    ...
    for (; it != indexes.end(); ++it) {
        if ((*it).column() == 0) {
            ...
            // type
            if (it->sibling(it->row(), 2).data().toString() == "Drive") {
...
bool FileSystemModel::deleteTarget(const QString &fileName)
{
    ...
    if (info.exists() || info.isShortcut()) {
        ...
        std::function<bool(const QString &)> moveToTrash = [&](const QString &filePath)
        {
            QFileInfo fileInfo(filePath);
            //QString trashDirPath = QStandardPaths::writableLocation(QStandardPaths::TrashLocation);
            //QString trashDirPath = QDesktopServices::storageLocation(QDesktopServices::TrashLocation);
            //QString trashDirPath = QStandardPaths::writableLocation(QStandardPaths::TrashLocation);
            auto getTrashDirPath = [](void)
            {
                QString homeDir = QStandardPaths::writableLocation(QStandardPaths::HomeLocation);
                QString trashDirPath = homeDir + "/.local/share/Trash/files";
                return trashDirPath;
            };
            // 获取系统回收站路径
            QString trashDirPath = getTrashDirPath();
            QString newFilePath = trashDirPath + "/" + fileInfo.fileName();
