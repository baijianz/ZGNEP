...
void FindWidget::showFiles(const QStringList &paths)
{
    for (const QString &filePath : paths) {
        ...
        std::function<QString(qint64)> formattedDataSize = [&](qint64 size)
        {
            QStringList sizes = {"Bytes", "KB", "MB", "GB", "TB"};
            int i = 0;
            double byteSize = size;

            while (byteSize >= 1024 && i < sizes.size() - 1) {
                byteSize /= 1024;
                ++i;
            }

            return QString().setNum(byteSize, 'f', 2) + " " + sizes.at(i);
        };
        QTableWidgetItem *sizeItem = new QTableWidgetItem(formattedDataSize(size));
