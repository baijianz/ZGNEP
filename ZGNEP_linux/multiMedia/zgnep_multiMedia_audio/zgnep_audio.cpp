...
zgnep_audio::zgnep_audio(QWidget *parent, QString dir, QString file)
    : QMainWindow(parent)
{
    /* 布局初始化 */
    musicLayout();
...
void zgnep_audio::musicLayout()
{
    ...
    /* 标签 */
    for (int i = 0; i < 4; i++)
        if(i != 1) label[i] = new QLabel();
        else label[i] = new zgnep_rotatingLabel();
    ...
    QSpacerItem *vSpacer1 = new
            QSpacerItem(310, 30,
                        QSizePolicy::Minimum,
                        QSizePolicy::Minimum);
    QSpacerItem *newvSpacer = new
            QSpacerItem(30, 30,
                        QSizePolicy::Minimum,
                        QSizePolicy::Minimum);
//    vBoxLayout[0]->addWidget(btnExit);
//    vBoxLayout[0]->addWidget(label[0]);

    QWidget *newHwidget = new QWidget();
    QHBoxLayout *newHlayout = new QHBoxLayout();

    /* 设置退出按钮 */
    btnExit = new  QPushButton();
    newHlayout->addWidget(btnExit);
    btnExit->setStyleSheet("background-image:url(:/images/btn_exit.png);");
    btnExit->setText("退出");
    QFont newfont = btnExit->font();
    newfont.setPointSize(10);
    QPalette newpal = btnExit->palette();
    newpal.setColor(QPalette::ButtonText, Qt::white);
    btnExit->setFont(newfont);
    btnExit->setPalette(newpal);

    newHlayout->addSpacerItem(newvSpacer);
    newHlayout->addWidget(label[0]);
    newHwidget->setLayout(newHlayout);

    vBoxLayout[0]->addWidget(newHwidget);
    ...
    QImage Image;
    Image.load("./multiMedia/zgnep_multiMedia_audio/images/cd.png");
...
//    vBoxLayout[1]->addWidget(durationSlider);
    ...
    hWidget[3]->setMinimumSize(320, 20);
    vBoxLayout[2]->addSpacerItem(vSpacer5);
    vBoxLayout[2]->addWidget(durationSlider);
    vBoxLayout[2]->addSpacerItem(vSpacer5);
    ...
    /* 设置标签文本 */
//    label[0]->setText(QCoreApplication::applicationDirPath()
//                      + "zgnep_audio");
    label[0]->setText("zgnep_audio音乐播放器");
...
zgnep_audio::~zgnep_audio()
{
}

void zgnep_audio::btn_play_clicked()
{
...
void zgnep_audio::btn_next_clicked()
{
    musicPlayer->stop();
...
void zgnep_audio::btn_previous_clicked()
{
    musicPlayer->stop();
    int count = mediaPlaylist->mediaCount();
    if (0 == count)
...
void zgnep_audio::btnExit_clicked()
{
...
void zgnep_audio::mediaPlayerStateChanged(
        QMediaPlayer::State
        state)
{
    switch (state) {
    case QMediaPlayer::StoppedState:
        ...
        dynamic_cast<zgnep_rotatingLabel *>(label[1])->setRotating(false);
        break;

    case QMediaPlayer::PlayingState:
        pushButton[1]->setChecked(true);
        dynamic_cast<zgnep_rotatingLabel *>(label[1])->setRotating(true);
        break;
...
void zgnep_audio::listWidgetCliked(QListWidgetItem *item)
{
    musicPlayer->stop();
...
void zgnep_audio::mediaPlaylistCurrentIndexChanged(
        int index)
{
    if (-1 == index)
        return;
...
void zgnep_audio::musicPlayerDurationChanged(
        qint64 duration)
{
...
void zgnep_audio::mediaPlayerPositionChanged(
        qint64 position)
{
...
void zgnep_audio::resizeEvent(QResizeEvent *event)
{
    Q_UNUSED(event);
    listMask->setGeometry(0,
                          listWidget->height() - 50,
...
void zgnep_audio::durationSliderReleased()
{
    /* 设置媒体播放的位置 */
    musicPlayer->setPosition(durationSlider->value() * 1000);
}

void zgnep_audio::scanSongs(QString path, QString file)
{
    QDir dir;
    if(!path.size())
    {
        dir.setPath(QCoreApplication::applicationDirPath()
             + "/multiMedia/zgnep_multiMedia_audio/myMusic");
    }
    else
    {
        dir.setPath(path);
        ...
        for (int i = 0; i < files.count(); i++) {
            ...
            if (mediaPlaylist->addMedia(
                ...
                if(info.filePath == file)
                {
                    ...
                    emit mediaPlaylistCurrentIndexChanged(i);
                }
            } else {
                qDebug()<<
...
void zgnep_audio::mediaPlayerInit()
{
    //CD动画
    animation = new QPropertyAnimation(label[1], "rotation"); //旋转的属性
