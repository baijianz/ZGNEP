...
zgnep_video::zgnep_video(QWidget *parent, QString dir, QString path)
    : QMainWindow(parent)
{
    /* 视频播放器布局初始化 */
    videoLayout();
...
zgnep_video::~zgnep_video()
{
}

void zgnep_video::videoLayout()
{
    ...
    btnExit->setMaximumSize(38, 38);
    btnExit->setMinimumSize(38, 38);
    ...
    QSpacerItem *hSpacer0 = new
            QSpacerItem(300, 80,
                        ...
                        QSizePolicy::Maximum);
    hBoxLayout[2]->addWidget(btnExit);
...
void zgnep_video::mediaPlayerInit()
{
...
void zgnep_video::btn_exit_clicked()
{
...
void zgnep_video::resizeEvent(QResizeEvent *event)
{
    Q_UNUSED(event);
    vWidget[1]->setGeometry(0, this->height() - 80, this->width(), 80);
}

void zgnep_video::btn_play_clicked()
{
...
void zgnep_video::btn_next_clicked()
{
    videoPlayer->stop();
...
void zgnep_video::btn_volmeup_clicked()
{
    /* 点击每次音量+5 */
...
void zgnep_video::btn_fullscreen_clicked()
{
    /* 全屏/非全屏操作 */
    vWidget[0]->setVisible(!pushButton[4]->isChecked());
}

void zgnep_video::btn_volmedown_clicked()
{
    /* 点击每次音量-5 */
    volumeSlider->setValue(volumeSlider->value() - 5);
    videoPlayer->setVolume(volumeSlider->value());
...
void zgnep_video::mediaPlayerStateChanged(
        QMediaPlayer::State
        state)
{
    switch (state) {
...
void zgnep_video::listWidgetCliked(QListWidgetItem *item)
{
    videoPlayer->stop();
...
void zgnep_video::mediaPlaylistCurrentIndexChanged(
        int index)
{
    if (-1 == index)
        return;
...
void zgnep_video::musicPlayerDurationChanged(
        qint64 duration)
{
...
void zgnep_video::mediaPlayerPositionChanged(
        qint64 position)
{
...
void zgnep_video::durationSliderReleased()
{
    /* 设置媒体播放的位置 */
    videoPlayer->setPosition(durationSlider->value() * 1000);
}

void zgnep_video::volumeSliderReleased()
{
    /* 设置音量 */
    videoPlayer->setVolume(volumeSlider->value());
}

void zgnep_video::scanVideoFiles(QString pdir, QString path)
{
    QDir dir, dirbsolutePath;
    if(!pdir.size())
    {
         dir.setPath(QCoreApplication::applicationDirPath()
             + "/multiMedia/zgnep_multiMedia_video/myVideo");
         dirbsolutePath.setPath(dir.absolutePath());
    }
    else
    {
        dir.setPath(pdir);
