...
{
    //zgnep_msg::zgnep_MsgBox("这是一个小小的测    this->setWindowState(this->windowState() | Qt::WindowFullScreen); // 将窗口设置为全屏模式试", MSG_TYPE::SA_OKS, MSG_TIP_TYPE::SA_QUESTION, this);
...
{
    zgnep_msg::zgnep_MsgBox("后续版本开放", MSG_TYPE::SA_OKS, MSG_TIP_TYPE::SA_TIPS, this);
}
...
void ZGNEP_Main::on_zgenp_main_text_clicked()
{
    ...
    this->hide();
    this->lower();
    zgnep_text *mode = new zgnep_text(this);
    app->setActiveWindow(mode);
    mode->show();
}
